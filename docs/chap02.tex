...
\end{tikzpicture}
\caption{Skill 语义地形模型示意图。XY 平面坐标由 Description embedding 经降维后确定，Z 轴高度对应 SkillRank 分值。两个语义聚类分别代表不同功能领域，红点为各聚类内的高质量 Skill。}
\label{fig:terrain}
\end{figure}
...
将这一思想迁移至 Skill 价值评估，关键的障碍是：Skill 之间不存在显式的超链接关系。ClawHub 上的 2 万余个 Skill 是相互独立发布的，没有任何一个 Skill 主动"链接"至另一个 Skill。
...
\begin{tikzpicture}[
  ...
  % 节点
  \node[highskill] (A) at (0, 0) {$s_1$\\{\tiny SR=0.91}};
  \node[skill] (B) at (3, 1.5) {$s_2$\\{\tiny SR=0.63}};
  \node[skill] (C) at (3, -1.5) {$s_3$\\{\tiny SR=0.57}};
  \node[skill] (D) at (6, 2.5) {$s_4$\\{\tiny SR=0.41}};
  \node[skill] (E) at (6, 0.5) {$s_5$\\{\tiny SR=0.38}};
  \node[skill] (F) at (6, -1.5) {$s_6$\\{\tiny SR=0.35}};
  \node[skill] (G) at (-3, 1) {$s_7$\\{\tiny SR=0.44}};
  \node[skill] (H) at (-3, -1) {$s_8$\\{\tiny SR=0.39}};
...
\begin{tabular}{lccc}
\toprule
方案 & 图构建复杂度 & 单轮迭代复杂度 & 增量更新复杂度 \\
\midrule
朴素全连接 & $O(n^2 d)$ & $O(n^2)$ & $O(n)$（重算全图） \\
SkillRank（FAISS K-NN） & $O(nd + n\log n)$ & $O(nK)$ & $O(K\log n)$（局部更新） \\
\bottomrule
\end{tabular}
\end{table}
